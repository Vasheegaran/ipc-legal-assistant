...
class CompleteIPCRAG:
    ...
    def setup_components(self):
        ...
        try:
            import groq
            # Import from config or use direct key
            try:
                from src.config import GROQ_API_KEY
                api_key = GROQ_API_KEY
            except:
                # Use direct key as fallback
                api_key = "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"
        ...
        try:
            ...
            if self.searcher.load_knowledge_base("knowledge_base/ipc_complete"):
                print("✅ IPC knowledge base loaded successfully")
            else:
                print("❌ Failed to load IPC knowledge base")
                self.searcher = None
...
COMPREHENSIVE IPC ANSWER:"""
    ...
    def ask(self, query):
        ...
        if not self.searcher:
            return "IPC search system not available. Knowledge base may not be loaded."
        if not self.client:
        ...
        print(f"📚 Found relevant IPC sections")
